...
def initialize_database() -> None:
    with closing(connect()) as db:
        ...
        company_table_sql = db.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='demo_company'").fetchone()
        if company_table_sql and "CHECK(id=1)" in company_table_sql[0].replace(" ", ""):
            db.execute("ALTER TABLE demo_company RENAME TO demo_company_legacy")
            db.execute("""CREATE TABLE demo_company (
                id INTEGER PRIMARY KEY, name TEXT NOT NULL, profile_json TEXT NOT NULL
            )""")
            db.execute("INSERT INTO demo_company SELECT id,name,profile_json FROM demo_company_legacy")
            db.execute("DROP TABLE demo_company_legacy")
        ...
        blue_profile = {
            "companyType": "주식회사", "classification": "중소기업", "foundedDate": "2022-06-10",
            "headOffice": "부산광역시 강서구", "workplaces": "부산광역시 강서구 미음산업단지",
            "industry": "산업용 센서 및 제어장치 제조업", "industryCode": "C27216",
            "products": "스마트팩토리 IoT 센서, 설비 모니터링 장치", "operatingStatus": "계속사업",
            "revenue": 2100000000, "employees": 18, "nationalTaxArrears": "없음",
            "localTaxArrears": "없음", "insuranceArrears": "없음", "sanctions": "없음",
            "supportHistory": "", "supportNeeds": ""
        }
        black_profile = {
            "companyType": "주식회사", "classification": "중소기업", "foundedDate": "2018-03-22",
            "headOffice": "경기도 성남시 분당구", "workplaces": "경기도 성남시 수정구",
            "industry": "인공지능 소프트웨어 개발 및 공급업", "industryCode": "J58222",
            "products": "기업용 데이터 분석 플랫폼, 생성형 AI 업무 솔루션", "operatingStatus": "계속사업",
            "revenue": 7200000000, "employees": 46, "nationalTaxArrears": "없음",
            "localTaxArrears": "없음", "insuranceArrears": "없음", "sanctions": "없음",
            "supportHistory": "2025년 AI 바우처 공급기업 사업 참여", "supportNeeds": "AI 연구개발과 해외 진출 지원",
            "currentProblems": "산업별 AI 모델 정확도 향상과 일본 시장 진출이 필요합니다.",
            "supportPriorities": "AI R&D, 해외 인증, 수출 마케팅", "businessGoals": "2년 내 매출 100억원과 해외 고객 20개사 확보",
            "goalPeriod": "2027년까지", "targetRevenue": 10000000000, "targetExport": 2500000000,
            "targetEmployees": 60, "desiredSupportAmount": 500000000, "maxContribution": 150000000,
            "availableStaff": 8, "targetMarkets": "일본 및 동남아시아 제조기업",
            "fundUsagePlan": "산업 특화 모델 개발, 현지 인증 및 전시회 참가", "applicationIntent": "높음"
        }
        yellow_profile = {
            "companyType": "주식회사", "classification": "소기업", "foundedDate": "2020-09-15",
            "headOffice": "전라북도 전주시 덕진구", "workplaces": "전라북도 익산시 국가식품클러스터",
            "industry": "건강기능식품 제조업", "industryCode": "C10797",
            "products": "지역 농산물 기반 건강식품, 기능성 음료", "operatingStatus": "계속사업",
            "revenue": 1600000000, "employees": 14, "nationalTaxArrears": "없음",
            "localTaxArrears": "없음", "insuranceArrears": "없음", "sanctions": "없음",
            "supportHistory": "2024년 지역특화 식품개발 사업 참여", "supportNeeds": "생산 자동화와 온라인 판로 확대",
            "currentProblems": "생산량 확대를 위한 자동화 설비와 전국 유통망이 부족합니다.",
            "supportPriorities": "생산설비, 제품 인증, 온라인 마케팅", "businessGoals": "18개월 내 매출 30억원과 신규 고용 6명",
            "goalPeriod": "2028년 상반기까지", "targetRevenue": 3000000000, "targetExport": 300000000,
            "targetEmployees": 20, "desiredSupportAmount": 300000000, "maxContribution": 80000000,
            "availableStaff": 5, "targetMarkets": "국내 온라인 시장 및 싱가포르",
            "fundUsagePlan": "포장 자동화 설비 도입, HACCP 고도화, 온라인 광고", "applicationIntent": "높음"
        }
        demo_companies = {
            1: ("더미그린", demo_profile), 2: ("더미레드", {}),
            3: ("더미블루", blue_profile), 4: ("더미블랙", black_profile),
            5: ("더미옐로", yellow_profile),
        }
        for company_id, (name, profile) in demo_companies.items():
            db.execute("""INSERT INTO demo_company(id,name,profile_json) VALUES(?,?,?)
                ON CONFLICT(id) DO UPDATE SET name=excluded.name""",
                (company_id, name, json.dumps(profile, ensure_ascii=False)))
        ...
        demo_accounts = [
            (1, "green@demo.local", "김지원", "더미그린"),
            (2, "red@demo.local", "레드관리자", "더미레드"),
            (3, "blue@demo.local", "블루관리자", "더미블루"),
            (4, "black@demo.local", "블랙관리자", "더미블랙"),
            (5, "yellow@demo.local", "옐로관리자", "더미옐로"),
        ]
        for company_id, email, display_name, company_name in demo_accounts:
            db.execute("""INSERT INTO companies(id,name) VALUES(?,?)
                ON CONFLICT(id) DO UPDATE SET name=excluded.name""", (company_id, company_name))
            db.execute("""INSERT INTO users(id,email,display_name,password) VALUES(?,?,?,'demo1234')
                ON CONFLICT(id) DO UPDATE SET email=excluded.email,display_name=excluded.display_name""",
                (company_id, email, display_name))
            db.execute("INSERT OR IGNORE INTO company_memberships(user_id,company_id,role) VALUES(?,?,'ADMIN')",
...
BASIC_REQUIRED_FIELDS = {
    "companyType": "기업 형태", "classification": "기업 구분", "foundedDate": "설립일",
    "headOffice": "본사 소재지", "industry": "업종", "industryCode": "주업종코드",
    "products": "주요 제품·서비스", "operatingStatus": "사업자 상태",
    "revenue": "최근 매출액", "employees": "상시근로자 수",
    "nationalTaxArrears": "국세 체납 여부", "localTaxArrears": "지방세 체납 여부",
    "insuranceArrears": "4대보험 체납 여부", "sanctions": "참여 제한·제재 여부",
}
...
def decide(profile: CompanyProfile, announcement: sqlite3.Row) -> dict[str, Any]:
    failures: list[str] = []
    passes: list[str] = []
    missing_basic = [label for field, label in BASIC_REQUIRED_FIELDS.items()
    ...
    if missing_basic:
        ...
        label = "신청 판단 불가능"
        reasons = [f"기본 기업정보가 부족합니다: {', '.join(missing_basic)}"]
    elif failures:
    ...
    return {"announcementId": announcement["id"], "title": announcement["title"],
            ...
            "eligibility": eligibility, "label": label, "reasons": reasons,
    ...
    try:
        ...
        with closing(connect()) as db:
            existing_ids = {row[0] for row in db.execute(
                "SELECT external_id FROM announcements WHERE external_id IS NOT NULL")}
        for source in latest_items:
            ...
            if len(source_items) >= IMPORT_LIMIT:
                break

        analyses = analyze_with_ai(source_items) if source_items else []
        ...
        message = ("공고 1건을 분석해 저장했습니다." if imported
                   else "최근 공고를 모두 확인했지만 새 공고가 없습니다.")
        with closing(connect()) as db:
            db.execute("""UPDATE import_jobs SET status='SUCCEEDED', fetched=?, imported=?,
                ...
                (len(latest_items), imported, skipped, message, job_id))
            db.commit()
...
def login(request: LoginRequest) -> dict[str, Any]:
    ...
    access_token = "demo-session" if row["company_id"] == 1 else f"demo-{row['company_id']}-session"
    with closing(connect()) as db:
        db.execute("INSERT OR REPLACE INTO auth_sessions(token,user_id,company_id) VALUES(?,?,?)",
                   (access_token, row["id"], row["company_id"]))
        db.commit()
    return {"accessToken": access_token, "displayName": row["display_name"],
            "companyName": row["name"], "role": row["role"]}

@app.get("/api/profile")
def get_profile(account: sqlite3.Row = Depends(current_account)) -> dict[str, Any]:
    with closing(connect()) as db:
        row = db.execute("SELECT name,profile_json FROM demo_company WHERE id=?",
                         (account["company_id"],)).fetchone()
    return {"companyName": row["name"], **json.loads(row["profile_json"])}

@app.put("/api/profile")
def put_profile(profile: CompanyProfile, account: sqlite3.Row = Depends(current_account)) -> dict[str, Any]:
    with closing(connect()) as db:
        db.execute("UPDATE demo_company SET profile_json=? WHERE id=?",
                   (profile.model_dump_json(), account["company_id"]))
        db.commit()
    return {"saved": True}

@app.get("/api/members")
def members(account: sqlite3.Row = Depends(current_account)) -> list[dict[str, str]]:
    with closing(connect()) as db:
        rows = db.execute("""SELECT u.display_name,u.email,m.role
            FROM company_memberships m JOIN users u ON u.id=m.user_id
            WHERE m.company_id=? ORDER BY u.id""", (account["company_id"],)).fetchall()
    return [{"displayName": row["display_name"], "email": row["email"],
             "role": row["role"], "joinedAt": "2026-09-03"} for row in rows]

@app.post("/api/matches")
def matches(profile: CompanyProfile | None = None,
            account: sqlite3.Row = Depends(current_account)) -> dict[str, Any]:
    if profile is None:
        with closing(connect()) as db:
            stored = db.execute("SELECT profile_json FROM demo_company WHERE id=?",
                                (account["company_id"],)).fetchone()[0]
    ...
    return {"total": len(results), "eligible": sum(r["eligibility"] == "ELIGIBLE" for r in results),
            "ineligible": sum(r["eligibility"] == "INELIGIBLE" for r in results),
            "undetermined": sum(r["eligibility"] == "UNDETERMINED" for r in results),
            "items": results}
